...
export const app = createApp({
  setup() {
    ...
    const checkGameOver = () => {
      ...
      if (
        player1Pits.every((pit) => pit === 0) ||
        player2Pits.every((pit) => pit === 0)
      ) {
        gameOver.value = true;
          ...
          standings:
            winner.value === 0
              ...
              : [
                  ...
                  {
                    place: 2,
                    user: winner.value === 1 ? 'Player 2' : 'Player 1',
                  },
                ],
        });
      }
    ...
    const makeMove = (pitIndex) => {
      ...
      if (!validPitRange.includes(pitIndex) || board.value[pitIndex] === 0)
        return;
      ...
      while (stones > 0) {
        ...
        if (
          (isPlayer1Turn && currentPit === 13) ||
          (!isPlayer1Turn && currentPit === 6)
        ) {
          continue;
        }
      ...
      if (!shouldRepeatTurn) {
        ...
        if (captureCondition) {
          ...
          if (board.value[oppositePit] > 0) {
            if (isPlayer1Turn) {
              board.value[6] +=
                board.value[lastPitLanded] + board.value[oppositePit] + 1;
            } else {
              board.value[13] +=
                board.value[lastPitLanded] + board.value[oppositePit] + 1;
            }
