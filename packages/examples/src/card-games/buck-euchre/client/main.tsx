...
export const game = {
  ...
  turn: {
    onBegin: ({
      ...
      G: BuckEuchreState;
      events: { setActivePlayers: (opts: unknown) => void };
    }) => {
      if (G.bidding) {
    ...
    scoring: {
      moves: {
        startNewRound: ({
          ...
          ctx: { numPlayers: number };
          random: { Shuffle: <T>(arr: T[]) => T[] };
          events: { setPhase: (phase: string) => void; endTurn: () => void };
        }) => {
    ...
    <div className="flex flex-col items-center w-full h-full pt-4 bg-white text-slate-900">
      ...
      <div className="flex flex-col items-center gap-2 mt-8 mb-12 px-4">
            ...
            return (
              ...
              >
                <span className="font-semibold flex items-center gap-1">
                  {`P${idx}`}
                  {isCurrentPlayer && (
                    <span className="ml-0.5 w-1.5 h-1.5 rounded-full bg-slate-800 animate-pulse" />
                  )}
                </span>
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-slate-400">
            <span>Trump</span>
            {G.trumpSuit ? (
              <span className={`${getSuitColor(G.trumpSuit)} text-sm`}>
                {getSuitIcon(G.trumpSuit)}
              </span>
            ) : (
      ...
      {canSelectTrump && (
        <div className="flex flex-col items-center mb-6">
          ...
          <button
            onClick={() => moves.exchangeWidow()}
            className="px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white bg-slate-900 rounded-md hover:bg-slate-800"
          >
            Exchange with Widow
          </button>
        </div>
        ...
        <div className="flex flex-col items-center w-full gap-4 p-4 mb-6">
            ...
            <div className="flex flex-col items-center w-full max-w-md gap-2 rounded-xl border border-slate-200 px-4 py-3">
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-600 border border-slate-300 rounded-md hover:bg-slate-50"
                >
                  Pass
                </button>
              </div>
          ...
          {canStartNewRound && (
            <button
              ...
              className="px-6 py-3 text-xs font-semibold uppercase tracking-[0.2em] text-white bg-slate-900 rounded-md hover:bg-slate-800"
            >
              Start New Round
            </button>
          )}
