...
describe('ClientAPI', () => {
  ...
  describe('constructor', () => {
    it('should remove trailing slash from base URL', () => {
      ...
      expect((apiWithSlash as unknown as { baseUrl: string }).baseUrl).toBe(
        'https://api.test.com',
      );
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('should throw on network error', async () => {
      ...
      await expect(api.getAppData('2025-07-01')).rejects.toThrow(
        'Network error',
      );
    });
  });
  ...
  describe('getAppId', () => {
    it('should throw when app ID is not set', async () => {
      const apiNoApp = new ClientAPI('https://api.test.com', mockUser);
      await expect(apiNoApp.getAppData('2025-07-01')).rejects.toThrow(
        'App ID not set',
      );
    });
  });
