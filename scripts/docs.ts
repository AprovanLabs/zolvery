 ...
 */
const parseJsDoc = (content: string): { description: string; usage?: string } => {
  ...
  const usage = usageIdx >= 0 ? lines[usageIdx].replace(/^usage:\s*/i, "") : undefined;
...
const parseYargsOptions = (content: string): ToolParameter[] => {
  ...
  while ((match = optionRegex.exec(content)) !== null) {
    ...
    const getField = (field: string): string | undefined => {
      const fieldMatch = optionsStr.match(new RegExp(`${field}:\\s*['"]?([^,'"\n]+)['"]?`));
      return fieldMatch?.[1]?.trim();
    };

    const getBoolField = (field: string): boolean => {
      const fieldMatch = optionsStr.match(new RegExp(`${field}:\\s*(true|false)`));
...
const tools = scripts.map(generateToolDef).filter((t): t is ToolDefinition => t !== null);
