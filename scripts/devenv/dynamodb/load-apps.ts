...
/**
 * Recursively find all kossabos.json files
 */
function findKossabosFiles(dir: string): string[] {
  const files: string[] = [];
...
/**
 * Load and parse a kossabos.json file
 */
...
async function main(): Promise<void> {
  console.log('🎮 Loading Kossabos apps into DynamoDB...');
  ...
  // Find all kossabos.json files
  const jsonFiles = findKossabosFiles(EXAMPLES_DIR);
  console.log(`📋 Found ${jsonFiles.length} kossabos.json files`);
  
  if (jsonFiles.length === 0) {
    console.log('⚠️  No kossabos.json files found. Exiting.');
    return;
  }
...
export { findKossabosFiles, loadAppConfig, createDynamoRecord };