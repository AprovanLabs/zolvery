...
describe("AppService", () => {
  ...
  describe("getAppById", () => {
    it("should throw NotFoundError when app store throws NotFoundError", async () => {
      mockGetApp.mockRejectedValue(new NotFoundError("App not found: test-app"));

      await expect(service.getAppById("test-app", "user-1")).rejects.toThrow(NotFoundError);
    });
    ...
    it("should throw UnauthorizedError when user is not the author of private app", async () => {
      ...
      await expect(service.getAppById("private-app", "user-1")).rejects.toThrow(UnauthorizedError);
    });
  });
});
