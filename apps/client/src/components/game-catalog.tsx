  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-medium text-sm text-slate-900 truncate">{game.name ?? game.appId}</h3>
        {game.description && (
          <p className="text-xs text-slate-500 line-clamp-2 mt-0.5">{game.description}</p>
        )}
      </div>
    </button>
  );
...
export function GameCatalog({ categories, isLoading, onSelectGame }: GameCatalogProps) {
  if (isLoading) {
    return (
    ...
    <div className="h-full overflow-auto">
      <div className="p-4 space-y-6 max-w-lg mx-auto">
          ...
          <section key={category.id} className="space-y-2">
            ...
            <div className="grid gap-2">
              {category.games.map((game) => (
                <GameCard key={game.appId} game={game} onClick={() => onSelectGame(game)} />
              ))}
            </div>
          </section>
        ))}
        ...
        <footer className="pt-4 border-t border-slate-100 text-center">
          <p className="text-xs text-slate-400 mb-2">Want more games? Build your own!</p>
          <a
